...
begin
  //Output a string
  vt := StringReplace(StringReplace(
    StringReplace(vText, '\', '\\', [rfReplaceAll]), ')', '\)', [rfReplaceAll]),
    '(', '\(', [rfReplaceAll]);
  sss := 'BT ' + FloatToStr(vX) + ' -' + FloatToStr(vY) + ' Td (' + vt + ') Tj ET';
  if ((Self.pUnderlineFlag) and (vt <> '')) then
    sss += ' ' + _dounderline(vX, vY, vt);
